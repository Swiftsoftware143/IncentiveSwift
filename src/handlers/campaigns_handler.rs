//! Campaigns handler — REST endpoints for campaigns.
...
pub async fn list(
    ...
    let sql = format!(
        "SELECT id, account_id, name, created_at, updated_at FROM campaigns ORDER BY name LIMIT $1 OFFSET $2"
    );
    let items = sqlx::query_as::<_, Campaigns>(&sql)
        .bind(limit)
        .bind(offset)
        .fetch_all(&state.db)
        .await
...
pub async fn create(
    ...
    sqlx::query(&format!("INSERT INTO campaigns (id, account_id, name) VALUES ($1, $2, $3)"))
        .bind(id)
        .bind(account_id)
        .bind(&body.name)
        .execute(&state.db)
        .await?;
    let item = sqlx::query_as::<_, Campaigns>(
        &format!("SELECT id, account_id, name, created_at, updated_at FROM campaigns WHERE id = $1")
    )
    .bind(id)
    .fetch_one(&state.db)
    .await?;
...
pub async fn update(
    ...
    let row = sqlx::query(&format!("SELECT name FROM campaigns WHERE id = $1"))
        .bind(item_id)
        .fetch_optional(&state.db)
        .await?
        .ok_or_else(|| AppError::NotFound(format!("Item not found: {}", id)))?;
    let new_name = body.name.unwrap_or_else(|| row.get("name"));
    sqlx::query(&format!("UPDATE campaigns SET name = $1, updated_at = now() WHERE id = $2"))
        .bind(&new_name)
        .bind(item_id)
        .execute(&state.db)
        .await?;
...
pub async fn delete(
    ...
    let item_id = Uuid::parse_str(&id)
        .map_err(|_| AppError::BadRequest("Invalid item ID".to_string()))?;
    let result = sqlx::query(&format!("DELETE FROM campaigns WHERE id = $1"))
        .bind(item_id)
        .execute(&state.db)
        .await?;
