...
pub struct CampaignSecretCode {
    pub id: Uuid, pub campaign_id: Uuid, pub code: String, pub points: i32,
    pub max_uses: Option<i32>, pub uses_count: i32, pub is_active: bool,
...
pub struct CreateSecretCodeBody {
    pub code: String, pub points: Option<i32>, pub max_uses: Option<i32>,
    pub expires_at: Option<chrono::DateTime<chrono::Utc>>,
}

#[derive(Debug, Deserialize)]
pub struct UpdateSecretCodeBody {
    pub code: Option<String>, pub points: Option<i32>, pub max_uses: Option<i32>,
    pub is_active: Option<bool>, pub expires_at: Option<Option<chrono::DateTime<chrono::Utc>>>,
}

#[derive(Debug, Deserialize)]
pub struct RedeemSecretCodeBody {
    pub code: String, pub contact_id: Uuid,
}
...
pub async fn list_secret_codes(
    ...
    ).bind(campaign_id).fetch_all(&app.db).await
...
pub async fn create_secret_code(
    ...
    ).bind(campaign_id).bind(&code).bind(points)
    .bind(body.max_uses).bind(body.expires_at)
    .fetch_one(&app.db).await {
        Ok(sc) => Ok(ok(json!({"secret_code": sc}))),
        Err(sqlx::Error::Database(ref d)) if d.constraint()
            == Some("campaign_secret_codes_code_campaign_id_key") =>
            Err(crate::error::AppError::BadRequest("Code already exists for this campaign".into())),
...
pub async fn update_secret_code(
    ...
    ).bind(&b.code).bind(b.points).bind(b.max_uses)
    .bind(b.is_active).bind(b.expires_at).bind(code_id)
    .fetch_one(&app.db).await
    .map_err(|e| crate::error::AppError::Internal(format!("DB: {}", e)))?;
...
pub async fn redeem_secret_code(
    State(app): State<AppState>, Path(campaign_id): Path<Uuid>,
    ...
    ).bind(campaign_id).bind(&cu)
    .fetch_optional(&app.db).await
    .map_err(|e| crate::error::AppError::Internal(format!("DB: {}", e)))? {
        Some(s) => s,
        None => return Ok(ok(json!({"success":false,"points_awarded":0,
    ...
    ).bind(sc.id).bind(body.contact_id)
    .fetch_one(&app.db).await
    .map_err(|e| crate::error::AppError::Internal(format!("DB: {}", e)))?;
    ...
    ).bind(campaign_id).bind(body.contact_id).bind(sc.points)
    .execute(&app.db).await
    .map_err(|e| crate::error::AppError::Internal(format!("DB: {}", e)))?;
    ...
    let _ = sqlx::query(
        "INSERT INTO campaign_secret_code_redemptions
         ...
         VALUES ($1,$2,$3,$4)"
    ).bind(sc.id).bind(body.contact_id).bind(campaign_id).bind(sc.points)
    .execute(&app.db).await;

    let _ = sqlx::query(
        "UPDATE campaign_secret_codes SET uses_count = uses_count + 1 WHERE id=$1"
    ).bind(sc.id).execute(&app.db).await;

    let cur_pts = sqlx::query_scalar::<_, i32>(
        "SELECT COALESCE(points,0) FROM campaign_points_balance
         WHERE campaign_id=$1 AND contact_id=$2"
    ).bind(campaign_id).bind(body.contact_id)
    .fetch_one(&app.db).await.unwrap_or(0);

    let _ = crate::mechanics::milestone_engine::check_milestones(
        &app, &campaign_id, &body.contact_id, cur_pts
    ).await;
