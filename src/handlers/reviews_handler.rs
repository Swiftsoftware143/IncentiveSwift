//! Reviews handler — REST endpoints for reviews.
...
pub async fn list(
    ...
    let sql = "SELECT id, account_id, name, created_at, updated_at FROM reviews ORDER BY name LIMIT $1 OFFSET $2".to_string();
    let items = sqlx::query_as::<_, Reviews>(&sql)
        .bind(limit)
        .bind(offset)
        .fetch_all(&state.db)
        .await
...
pub async fn create(
    ...
    sqlx::query("INSERT INTO reviews (id, account_id, name) VALUES ($1, $2, $3)")
        ...
        .execute(&state.db)
        .await?;
    let item = sqlx::query_as::<_, Reviews>(
        "SELECT id, account_id, name, created_at, updated_at FROM reviews WHERE id = $1"
    )
    .bind(id)
    .fetch_one(&state.db)
    .await?;
...
pub async fn update(
    ...
    let row = sqlx::query("SELECT name FROM reviews WHERE id = $1")
        .bind(item_id)
        .fetch_optional(&state.db)
        .await?
