...
pub async fn list(
    ...
    let sql = "SELECT id, account_id, name, created_at, updated_at FROM surfaces ORDER BY name LIMIT $1 OFFSET $2".to_string();
    let items = sqlx::query_as::<_, Surfaces>(&sql)
        .bind(limit)
        .bind(offset)
        .fetch_all(&state.db)
        .await
...
pub async fn update(
    ...
    sqlx::query("UPDATE surfaces SET name = $1, updated_at = now() WHERE id = $2")
        .bind(&new_name)
        .bind(item_id)
        .execute(&state.db)
        .await?;
