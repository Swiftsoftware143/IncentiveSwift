//! Tickets handler — REST endpoints for tickets.
...
pub async fn list(
    ...
    let sql = "SELECT id, account_id, name, created_at, updated_at FROM tickets ORDER BY name LIMIT $1 OFFSET $2".to_string();
    let items = sqlx::query_as::<_, Tickets>(&sql)
        .bind(limit)
        .bind(offset)
        .fetch_all(&state.db)
        .await
...
pub async fn create(
    ...
    sqlx::query("INSERT INTO tickets (id, account_id, name) VALUES ($1, $2, $3)")
        .bind(id)
        .bind(account_id)
        .bind(&body.name)
        .execute(&state.db)
        .await?;
    let item = sqlx::query_as::<_, Tickets>(
        "SELECT id, account_id, name, created_at, updated_at FROM tickets WHERE id = $1"
    )
    .bind(id)
    .fetch_one(&state.db)
    .await?;
...
pub async fn update(
    ...
    let row = sqlx::query("SELECT name FROM tickets WHERE id = $1")
        .bind(item_id)
        .fetch_optional(&state.db)
        .await?
        .ok_or_else(|| AppError::NotFound(format!("Item not found: {}", id)))?;
    let new_name = body.name.unwrap_or_else(|| row.get("name"));
    sqlx::query("UPDATE tickets SET name = $1, updated_at = now() WHERE id = $2")
        .bind(&new_name)
        .bind(item_id)
        .execute(&state.db)
        .await?;
