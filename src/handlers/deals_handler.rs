//! Deals handler — REST endpoints for deals.
...
pub async fn list(
    ...
    let sql = "SELECT id, account_id, name, created_at, updated_at FROM deals ORDER BY name LIMIT $1 OFFSET $2".to_string();
    let items = sqlx::query_as::<_, Deals>(&sql)
        .bind(limit)
        .bind(offset)
        .fetch_all(&state.db)
        .await
...
pub async fn create(
    ...
    sqlx::query("INSERT INTO deals (id, account_id, name) VALUES ($1, $2, $3)")
        .bind(id)
        .bind(account_id)
        .bind(&body.name)
        .execute(&state.db)
        .await?;
    let item = sqlx::query_as::<_, Deals>(
        "SELECT id, account_id, name, created_at, updated_at FROM deals WHERE id = $1"
    )
    .bind(id)
    .fetch_one(&state.db)
    .await?;
...
pub async fn update(
    ...
    sqlx::query("UPDATE deals SET name = $1, updated_at = now() WHERE id = $2")
        .bind(&new_name)
        .bind(item_id)
        .execute(&state.db)
        .await?;
